...
(() => {
    ...
    const performSearch = () => {
        ...
        cards.forEach((card) => {
            ...
            if (titleEl && !titleEl.getAttribute('data-original'))
                titleEl.setAttribute('data-original', title);
            if (descEl && !descEl.getAttribute('data-original'))
                descEl.setAttribute('data-original', description);
            if (catEl && !catEl.getAttribute('data-original'))
                catEl.setAttribute('data-original', category);
            if (urlEl && !urlEl.getAttribute('data-original'))
                urlEl.setAttribute('data-original', url);
            ...
            if (normalizedQuery === '' || searchText.includes(normalizedQuery)) {
                ...
                if (query) {
                    ...
                    if (catEl)
                        catEl.innerHTML = highlightText(origCat, query);
                    if (urlEl)
                        urlEl.innerHTML = highlightText(origUrl, query);
                }
                else {
                    if (titleEl)
        ...
        sections.forEach((section) => {
            ...
            if (grid && grid.classList.contains('grid')) {
                const hasVisibleCards = Array.from(grid.querySelectorAll('.card')).some(c => c.style.display !== 'none');
